...
def read_gdp_data(happy_dict):
    ...
    with open('world_pop_gdp.tsv', 'r') as file:
        ...
        for line in file:
            ...
            # Skip countries with a population less than 100 million
            if population < 100:
                continue

            gdp_per_capita = format(float(data[2].replace('$', '').replace(',', '')), '.2f')

            # Look up happiness index in the dictionary
            happiness_index = happy_dict.get(country, None)

            # Print comma-separated output with formatted GDP per Capita
